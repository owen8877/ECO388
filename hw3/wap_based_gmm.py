...
class FullyConnectedNetwork(nn.Module):
    def __init__(self, input_dim: int, output_dim: int = 1, hidden_dim: int = 10, depth: int = 3,
                 activation: str = 'relu'):
        super().__init__()
        self.preprocess = nn.Linear(input_dim, hidden_dim)

        self.depth = depth
        self.transforms = nn.ModuleList([nn.Linear(hidden_dim, hidden_dim) for _ in range(self.depth)])
        ...
        if activation == "relu":
            ...
            self.activation = lambda x: Func.silu(x)
        else:
            raise Exception(f"Can't recognize activation {activation}!")
        self.activation_str = activation

    def forward(self, x):
        z_i = [self.preprocess(x)]
        for _ in range(self.depth):
            z = self.transforms[_](z_i[-1])
            z_i.append(self.activation(z))
        return self.fc(z_i[-1])


class Resnet(nn.Module):
    def __init__(self, input_dim: int, output_dim: int = 1, hidden_dim: int = 10, depth: int = 3,
                 block_per_group: int = 3, activation: str = 'relu'):
        super().__init__()
        self.preprocess = nn.Linear(input_dim, hidden_dim)
        ...
        self.transforms = nn.ModuleList(
            [nn.ModuleList([nn.Linear(hidden_dim, hidden_dim) for _ in range(self.block_per_group)]) for _ in
             range(self.depth)])
        self.fc = nn.Linear(hidden_dim, output_dim)
    ...
    def forward(self, x):
        z_i = [self.preprocess(x)]
        for group in self.transforms:
            z = z_i[-1]
            z_ii = [z]
            for block in group:
                z_ii.append(block(self.activation(z_ii[-1])))
            z_i.append(z_ii[-1]+z)
        return self.fc(z_i[-1])
...
class GMMApproach(Estimator):
    def __init__(self, train_df: DataFrame, save_path: str = None, hidden_dim: int = 10, depth: int = 3,
                 block_per_group: int = 3, activation: str = 'relu', batch_size: int = 100):
        super().__init__(train_df, 'gmma')
        # self.net = FullyConnectedNetwork(input_dim=3, hidden_dim=hidden_dim, depth=depth, activation=activation)
        self.net = Resnet(input_dim=3, hidden_dim=hidden_dim, depth=depth, block_per_group=block_per_group,
