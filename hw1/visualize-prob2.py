...
plt.plot(zs, cdf_kernel_estimator(zs, sub_sample, gaussian_cdf, np.power(10, -1 / 2)), ls=':', c=palette[0],
         label='10, smoothed')
...
plt.plot(zs, cdf_kernel_estimator(zs, sub_sample, gaussian_cdf, np.power(100, -1 / 2)), ls=':', c=palette[1],
         label='100, smoothed')

plot_empirical_distribution(wage_samples.values, plot_bounds, plot_kws={'color': palette[2], 'ls': '-'}, label='full')
plt.plot(zs, cdf_kernel_estimator(zs, wage_samples.values, gaussian_cdf, np.power(N, -1 / 2)), ls=':', c=palette[2],
         label='full, smoothed')
